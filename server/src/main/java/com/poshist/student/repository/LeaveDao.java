...
public interface LeaveDao   extends CrudRepository<Leave, Long>, JpaSpecificationExecutor<Leave> {
    ...
    @Query(nativeQuery = true, value = "select left(d.code,6),count(t.id) from t_st_leave_info t left join t_st_student_info u on t.student_id=u.id left join t_sys_department_info d on u.department_id=d.id  where t.estimate_start_time>=?1 and t.estimate_start_time<=?2 group by left(d.code,6)")
    List<Object[]> findCountLeaveDepartment(Date minDate, Date maxDate);
    @Query(nativeQuery = true, value = "select count(t.id) from t_st_leave_info t left join t_st_student_info u on t.student_id=u.id left join t_sys_department_info d on u.department_id=d.id  where d.code like CONCAT(?1,'%') and DATE_FORMAT(t.estimate_start_time,'%Y-%m-%d')<=?2 and DATE_FORMAT(t.estimate_end_time,'%Y-%m-%d')>=?2")
